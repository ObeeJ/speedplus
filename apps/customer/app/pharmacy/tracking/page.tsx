...
const STEPS = ['Order confirmed', 'Pharmacist packed and sealed it', 'On the way', 'Delivered'];
...
export default function PharmacyTrackingPage() {
  const { deliverToAddress, orderId, reset } = usePharmacyFlowStore();
  ...
  const deliverToLabel = deliverToAddress
    ? `${deliverToAddress.street}, ${deliverToAddress.city}`
    : null;
    ...
    <main className="min-h-screen bg-sand flex flex-col">
      <div className="bg-emerald relative overflow-hidden px-5 py-8 min-[700px]:px-9 min-[700px]:py-12 flex flex-col items-center gap-3">
        ...
        <span className="text-sm text-sand/70">
          {deliverToLabel ? `Arriving at ${deliverToLabel}` : 'On its way'}
        </span>
      </div>
      ...
      <div className="flex-1 px-5 py-6 min-[700px]:max-w-[860px] min-[700px]:mx-auto min-[700px]:px-8 min-[700px]:py-10 min-[700px]:w-full flex flex-col gap-6">
        <div className="bg-white border border-line rounded-2xl p-4 flex items-center gap-3">
          <span className="w-11 h-11 rounded-full bg-tile flex items-center justify-center font-display font-semibold text-emerald">B</span>
          <div className="flex-1 flex flex-col">
            <span className="font-semibold text-ink text-[14px]">Blessing</span>
            <span className="text-[12px] text-mid">Bike · ★ 4.9</span>
          </div>
          <button className="w-10 h-10 rounded-full bg-tile flex items-center justify-center" aria-label="Call rider">
        ...
        {deliverToLabel && (
          <span className="text-[13px] text-mid">
            Delivering to <b className="text-ink">{deliverToLabel}</b>
          </span>
        )}
            ...
            return (
              <div key={label} className="flex gap-3">
                <div className="flex flex-col items-center">
                  ...
                  {i < STEPS.length - 1 && (
                    <span className={`w-0.5 flex-1 min-h-[28px] ${done ? 'bg-emerald' : 'bg-line'}`} />
                  )}
                </div>
                <span className={`text-[14px] pb-7 ${done ? 'text-ink font-medium' : 'text-mid'}`}>
                  {label}
                </span>
              </div>
            );
