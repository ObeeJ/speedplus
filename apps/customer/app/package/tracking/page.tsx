...
export default function PackageTrackingPage() {
  const { pickup, dropoff, orderId, reset } = usePackageFlowStore();
  ...
  useEffect(() => {
    if (liveOrder) return; // real order data is driving state; skip the local demo simulation
    const stepTimer = setInterval(() => {
      setStepIndex((s) => Math.min(s + 1, STEPS.length - 1));
    }, 8000);
    const etaTimer = setInterval(() => {
      setEtaMin((m) => Math.max(m - 1, 0));
    }, 15000);
    return () => {
  ...
  return (
    <main className="min-h-screen bg-sand flex flex-col">
      <div className="bg-emerald relative overflow-hidden px-5 py-8 min-[700px]:px-9 min-[700px]:py-12 flex flex-col items-center gap-3">
        ...
        <span className="text-sm text-sand/70">{dropoff ? `Arriving at ${dropoff}` : 'On its way'}</span>
      </div>
      ...
      <div className="flex-1 px-5 py-6 min-[700px]:max-w-[860px] min-[700px]:mx-auto min-[700px]:px-8 min-[700px]:py-10 min-[700px]:w-full flex flex-col gap-6">
        <div className="bg-white border border-line rounded-2xl p-4 flex items-center gap-3">
          <span className="w-11 h-11 rounded-full bg-tile flex items-center justify-center font-display font-semibold text-emerald">M</span>
          <div className="flex-1 flex flex-col">
            <span className="font-semibold text-ink text-[14px]">Musa</span>
        ...
        {pickup && dropoff && (
          <span className="text-[13px] text-mid">
            From <b className="text-ink">{pickup}</b> to <b className="text-ink">{dropoff}</b>
          </span>
        )}
