...
import { giftCardsApi } from '@fourdat/api-client';
        ...
        <div className="flex gap-1 bg-white/10 rounded-xl p-1">
          {(['redeem', 'issue'] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`flex-1 rounded-lg py-2 text-[13px] font-semibold transition-all ${tab === t ? 'bg-white text-emerald' : 'text-white/60 hover:text-white'}`}
            >
          ...
          <>
            <ListCard className="flex flex-col gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[14px] font-semibold text-ink">Enter gift card code</p>
                  <p className="text-[12px] text-mid">Balance added to your wallet instantly</p>
                </div>
              </div>
              <form onSubmit={handleRedeem} className="flex flex-col gap-3">
                <input
                  ...
                  className="w-full border border-line rounded-xl px-4 py-3 text-[15px] font-mono text-ink placeholder-[#C5C2BB] tracking-widest focus:outline-none focus:border-emerald transition-colors"
                  maxLength={24}
                  aria-label="Gift card code"
                />
                {redeem.isError && <p className="text-xs text-red-600" role="alert">{(redeem.error as Error).message}</p>}
                {redeem.isSuccess && <p className="text-xs text-emerald font-semibold" role="status">✓ Card redeemed — balance added to your wallet</p>}
            ...
            {issued ? (
              <ListCard className="flex flex-col gap-4">
                <div className="flex flex-col items-center gap-3 py-4">
                  <div className="w-14 h-14 rounded-2xl bg-tile flex items-center justify-center">
                  ...
                  <p className="font-display font-bold text-[20px] text-ink">{naira(issued.amountKobo)} gift card</p>
                  <p className="text-[12px] text-mid">Share this code with the recipient</p>
                  <div className="w-full bg-sand rounded-xl px-4 py-3 flex items-center justify-between gap-3">
                    <p className="font-mono text-[16px] font-bold text-ink tracking-widest">{issued.code}</p>
                    <button onClick={copyCode} className={`text-[12px] font-semibold rounded-lg px-3 py-1.5 transition-all ${copied ? 'bg-tile text-emerald' : 'border border-line text-mid hover:border-emerald/30'}`}>
                      {copied ? 'Copied!' : 'Copy'}
                    </button>
                  </div>
                </div>
              ...
              <ListCard className="flex flex-col gap-4">
                <p className="text-[14px] font-semibold text-ink">Choose amount</p>
                <div className="grid grid-cols-2 gap-2">
                  {PRESETS.map((p) => (
                    <button
                      key={p}
                      onClick={() => setIssueAmount(String(p / 100))}
                      className={`rounded-xl border-2 py-3 text-[14px] font-semibold transition-all ${issueAmount === String(p / 100) ? 'border-emerald bg-tile text-emerald' : 'border-line text-mid hover:border-emerald/30'}`}
                    >
                      {naira(p)}
                    </button>
                  ))}
                ...
                <form onSubmit={handleIssue} className="flex flex-col gap-3">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="500"
                      className="w-full border border-line rounded-xl pl-8 pr-4 py-3 text-[15px] text-ink placeholder-[#C5C2BB] focus:outline-none focus:border-emerald transition-colors" aria-label="500"/>
                  </div>
